...
class Node(object):
    ...
    def serialize(self, indenter, first=False):
        """Returns the Node serialized as an xml string"""
        # Determine the start and end of this node
        wrap = self.wrap
        end, start = "", ""
        if wrap:
            end = "</%s>" % (wrap if first else textutils.Singularize(wrap))
            start = "<%s>" % (wrap if first else textutils.Singularize(wrap))
...
class Converter(object):
    ...
    def build(self, data):
        ...
        return Node(wrap=self.wrap, data=data).serialize(indenter, True)
